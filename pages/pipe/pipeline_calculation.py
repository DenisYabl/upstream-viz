...
def app():
    ...
    try:
        ...
        mdf = mdf.rename(columns={'L': "Длина участка", 'node_name_start': "Начало участка",
                                  'node_name_end': "Конец участка", 'uphillM': "Перепад высот, м.",
                                  'startP': "Давление в начале, атм.",
                                  'endP': "Давление в конце, атм.", 'D': "Диаметр, мм.",
                                  'res_watercut_percent': "Обводненность, %",
                                  'res_liquid_density_kg_m3': "Плотность, кг/м3", 'X_kg_sec': "Массовый расход, кг/сек",
                                  'velocity_m_sec': "Скорость потока, м/с"})

        st.success("Решение найдено")
        st.write(mdf.style.format(
            {"Длина участка": '{:.1f}', "Перепад высот, м.": '{:.1f}', "Давление в начале, атм.": '{:.1f}',
