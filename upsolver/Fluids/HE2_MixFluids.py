...
def evalute_network_fluids_wo_root(_G, x_dict):
    ...
    # the first partition of matrix is for 1stCL
    mx1 = np.zeros((N, M))
    # the second partition is to dictate condition: all fluids leaving one node have to be the same
    mx2 = np.zeros((M-N, M))
    i2 = 0
    for i, node in enumerate(nodes):
        ...
        if len(these_vars_are_the_same) > 1:  # if there are more than one remembered outlet flow
            for fl1, fl2 in zip(these_vars_are_the_same[1:], these_vars_are_the_same[:-1]):
                # we have to right an equation to matrix, to describe that these outlet fluids are equal
                mx2[i2, fl1] = 1
                mx2[i2, fl2] = -1
                i2 += 1
